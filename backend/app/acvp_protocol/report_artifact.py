...
REPORT_ARTIFACT_TYPE = "nccu-acvp-validation-report"
REPORT_DISCLAIMER = (
...
def append_report_artifact(
    *,
    ...
    """Append an immutable validation summary to a vector set's report history."""
...
def build_report_artifact(
    *,
    ...
    artifact: Dict[str, Any] = {
        ...
        "resultsSha256": sha256_json(acvp_results),
...
def latest_report_artifact(vector_set: Dict[str, Any]) -> Dict[str, Any] | None:
    ...
    if not artifacts:
        return report_store if report_store.get("reportId") else None
    latest_report_id = report_store.get("latestReportId")
    return next(
        (
            artifact
            for artifact in artifacts
            if artifact.get("reportId") == latest_report_id
        ),
        artifacts[-1],
    )
...
def _summary(validation_result: Dict[str, Any]) -> Dict[str, int]:
    ...
    return {
        ...
        for key in ("total", "passed", "failed", "missing", "malformed", "extra")
    }
